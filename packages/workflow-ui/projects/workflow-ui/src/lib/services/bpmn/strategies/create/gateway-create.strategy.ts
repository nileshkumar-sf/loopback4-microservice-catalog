...
export class CreateGatewayStrategy implements CreateStrategy<ModdleElement> {
  ...
  execute(
    ...
    // if (node.outgoing.split('__').length !== node.next?.length) {
    //   (element as GatewayElement).otherOutgoings = [];
    //   for (let i = 1; i < node.next?.length; i++) {
    //     (element as GatewayElement).otherOutgoings.push(`Flow_${this.utils.uuid()}`);
    //   }
    // }
    if(node.next.length > 1)
      (element as GatewayElement).otherOutgoings = Array(node.next.length -1).fill(`Flow_${this.utils.uuid()}`);
