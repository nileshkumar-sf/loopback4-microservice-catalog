...
export class CreateOrGatewayStrategy implements CreateStrategy<ModdleElement> {
  ...
  execute(
    ...
    (element as OrGatewayElement).elseOutGoing = (element as OrGatewayElement).elseOutGoing ?? `Flow_${this.utils.uuid()}`;
    (element as OrGatewayElement).default = (element as OrGatewayElement).default ?? `Flow_${this.utils.uuid()}`;
